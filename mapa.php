<html lang="es">
<head>
	<meta charset="utf-8"> <!--Cotejamiento-->
	<meta name="description" content=""> <!--Descripción del sitio-->
	<meta name="keywords" content=""> <!--Palabras clave-->
	<title>Making Satellite - Mapa del sitio</title> <!--Titulo del sitio-->
	<link rel="shortcut icon" type="../pictures/making/x-icon" href="../pictures/making/icon-2.png"> <!--Icono del sitio-->
	<link rel="stylesheet" type="text/css" href="css/mapa.css">
	<link rel="stylesheet" type="text/css" href="css/icons.css">
</head>

<body>
	<header>
		<font>Mapa del sitio</font>
		<a href="../index.html"><img src="pictures/making/logo.png"></a> <!--Logo de la empresa-->
		<br>
		<table class="logbar"> <!--Menú de login-->
			<tr>
				<td align="right"><a href="acceso.php">Iniciar sesión</a> &nbsp;|&nbsp; <a href="alta_usuario.php">Registrarme</a></td>
			</tr>
		</table>
		<br>
		<div class="contenedor">
		<nav class="menu-fixed"> <!--Menú de navegación-->
			<ul>
			<center>
				<li><a href="index.php">Inicio</a></li>
				<li><a href="ver_producto.php">Catálogo</a></li>
				<li><a href="#info">Contácto</a></li>
				<li><a href="mapa.php">Mapa del sitio</a></li>
                <li><a href="carrito_lista.php">Carrito</a></li>
			</center>
			</ul>
		</nav>
        </div>
	</header>

	<section>
		<span class="mapa"> <!--Mapa del sitio-->
			<ul><a href="../index.html">Inicio</a></ul>
			<ul><a href="catalogo.php">Catálogo</a></ul>
				<li><a href="camisetas.php">Camisetas</a></li>
				<li><a href="busos.php">Busos</a></li>
				<li><a href="pantalones.php">Pantalones</a></li>
				<li><a href="pantalonetas.php">Pantalonetas</a></li>
			<ul><a href="#info">Contácto</a></ul>
		</span>
		<div class="lado">
			<img src="pictures/making/bienvenida-2.png">
		</div>
	</section>

	<br>
	<footer> <!--Pie de página-->
		<div id="info"> <!--Datos de contácto-->
			<h3>Contácto</h3>
			<p>Dirección: Carrera 81G N° 73 - 57 Sur | Bogotá</p>
			<br>
			<p class="tel">Teléfono: 302 5171</p>
		</div>
		
		<div class="redes"> <!--Redes sociales-->
			<h3>Redes sociales</h3>
			<a target="_blank" href="https://www.facebook.com/"><img src="pictures/redes sociales/facebook.png" width="22px"></a>
			<a target="_blank" href="https://twitter.com/"><img src="pictures/redes sociales/twitter.png" width="25px"></a>
			<a target="_blank" href="https://www.instagram.com/"><img src="pictures/redes sociales/instagram.png" width="22px"></a>
			<a target="_blank" href="https://www.youtube.com/"><img src="pictures/redes sociales/youtube.png" width="25px" height="22px"></a>
		</div>
		<div class="admin">
		<a href="login-administrador.php" title="Ingresar como administrador">Administración</a></div> <!--Login de administrador-->
		<p class="derechos">© Making Satellite - 2016</p> <!--Derechos de autor-->
	</footer>

</body>

</html>